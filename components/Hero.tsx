 ...
 * zoom del navegador) el logo y el título caen a distinta altura de la foto,
 * y en algunas pantallas quedaban sobre la cara de quien está sentada al
 * centro. Acá se mide dónde empieza el texto y se desplaza la foto para que
...
function encuadrar(anchoCaja: number, altoCaja: number, topeTexto: number) {
  ...
  return `50% ${((desplazamiento / sobra) * 100).toFixed(2)}%`;
}
      ...
      >
        ...
        >
          <Image
            ...
            height={1238}
            priority
            className="h-11 w-auto sm:h-12 sm:[@media(max-height:760px)]:h-11"
          />
        </motion.div>

        <div className="mt-4 overflow-hidden [@media(max-height:760px)]:mt-3">
          <motion.h1
        ...
        >
          ...
          {/* En celulares de poca altura se muestra solo la consulta: con los
              dos botones apilados, el logo quedaba sobre una cara. */}
          <a
            href="#servicios"
